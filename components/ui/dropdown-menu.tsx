...
/**
 * Base UI has no DropdownMenu: the primitive is `Menu`, and it splits Radix's
 * `Content` into `Portal > Positioner > Popup`. Highlight state moved from DOM
 * focus (`focus:`) to `data-highlighted`, because Base UI keeps focus on the
 * trigger and moves an active-descendant marker instead.
 *
 * Popups sit on the `rounded-xl` container tier; the rows inside them step one
 * notch down to `rounded-md`, which is the standard nesting for this system.
 */
    ...
    <DropdownMenuPrimitive.Portal>
      ...
      >
        <DropdownMenuPrimitive.Popup
          ...
          className={cn(
            "z-50 max-h-(--available-height) min-w-[8rem] origin-(--transform-origin) overflow-x-hidden overflow-y-auto rounded-xl border bg-popover p-1.5 text-popover-foreground shadow-lift outline-none",
            className,
          )}
          {...props}
        />
...
function DropdownMenuCheckboxItem({
  ...
  return (
    <DropdownMenuPrimitive.CheckboxItem
      ...
      className={cn(
        "relative flex cursor-default items-center gap-2 rounded-md py-1.5 pr-2 pl-8 text-sm outline-hidden select-none data-highlighted:bg-accent data-highlighted:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className,
      )}
      checked={checked}
      {...props}
...
/** Radix `Sub` / `SubTrigger` are `SubmenuRoot` / `SubmenuTrigger` in Base UI. */
...
function DropdownMenuSubTrigger({
  ...
  return (
    <DropdownMenuPrimitive.SubmenuTrigger
      ...
      data-inset={inset}
      className={cn(
        // `data-popup-open` is the Base UI marker for "my submenu is showing";
        // there was no radix equivalent, which used data-state=open.
...
/**
 * Composes the public Content wrapper. The align/side offsets are load-bearing:
 * they line the submenu up with the row that opened it.
 */
