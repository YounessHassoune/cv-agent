        ...
        <div className="flex items-center gap-3">
          ...
          <Select
            onValueChange={(template) => {
              if (!template) return;
              // Locked layouts stay pickable and sell themselves; see the same
              // gesture on the application workspace.
              if (!templateAllowed(plan.plan, template)) {
                ...
                <SelectItem className="py-2" key={template.id} value={template.id}>
                  ...
                  <span className="flex flex-col gap-0.5">
                    <span className="flex items-center gap-1.5 font-medium">
                      {template.label}
                      {templateAllowed(plan.plan, template.id) ? null : <LockPill />}
                    </span>
          ...
          <CvThemePicker
            onChange={(theme) => {
              if (!themeAllowed(plan.plan, theme)) {
                plan.upgrade(theme.startsWith("#") ? "customThemeColor" : "allTemplates");
                return;
              }
              patch({ theme });
            }}
