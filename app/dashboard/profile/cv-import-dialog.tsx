...
export function CvImportDialog({
  ...
  /*
   * The lock is on the count, not the plan: every plan may import, and Free
   * gets one so that the best thing the product does is visible before anyone
   * pays for it. Once it is spent the button stays exactly where it is and
   * says what it would do — it just opens the upgrade dialog instead of the
   * file picker. A disabled button here would teach the user that the app is
   * broken, which is the one thing a paywall must never look like.
   */
  if (!plan.can("cvImport") || plan.cvImportsLeft <= 0) {
    return (
      <Button onClick={() => plan.upgrade("cvImport")} size="sm" type="button" variant="outline">
