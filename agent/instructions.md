...
- `compile_pdf` returns `unsupported`: the terms the CV claims that the profile does not list. **Name every one of them in the staging summary** and tell the user to confirm or strike them before applying. They are the user's assertion, not the profile's.
